...
export default function Subscription() {
  ...
  const [pixData, setPixData] = useState<PixData | null>(() => {
    // Restaura PIX pendente do localStorage ao recarregar
    try {
  ...
  // Email capturado no momento de gerar o PIX (imutável durante polling)
  ...
  // Se restaurou pixData do localStorage, vai direto para pix_qr
  useEffect(() => {
  ...
  // PIX polling (para quando expirar)
  useEffect(() => {
  ...
  // Preenche email quando o usuário já está logado
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (paymentMethod === "pix") {
        // Captura email imutável para o polling
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col items-center gap-6">
          ...
          {pixData.ticketUrl && (
            <a
              ...
              className="text-[13px] text-blue-500 underline"
            >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
          ...
          {paymentMethod === "pix" && (
            <div className="bg-[#32BCAD]/10 border border-[#32BCAD]/30 rounded-xl p-4 text-center space-y-1">
              <p className="text-[13px] font-semibold text-[#1a8c80]">
                Como funciona o PIX
              </p>
              <p className="text-[12px] text-gray-600 leading-relaxed">
                Após clicar em Gerar QR Code, você receberá um código para
                pagar pelo app do seu banco. A confirmação é instantânea.
              </p>
            </div>
          )}
    ...
    <div className="h-[100svh] bg-[#f3f4f6] flex flex-col overflow-hidden relative">
      {!isTrialExpired && (
        <div className="absolute top-6 right-6 z-50">
          <button
            onClick={handleClose}
            className="p-2 text-gray-500 bg-white/50 backdrop-blur-sm rounded-full"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="w-full grid grid-cols-2 gap-3 mb-4">
          {PLANS.map((plan) => (
            ...
            >
              ...
              <span className="text-[13px] font-bold text-[#1a1a1a] mb-0.5">
                {plan.name}
              </span>
